...
from app.api.dependencies import Operator, Reviewer
...
class Reason(StrictModel):
    ...
    def meaningful(cls,value):
        if len(value.strip())<5: raise ValueError("Give a review reason")
        return value.strip()
...
async def restore(email_id:UUID,body:Reason,request:Request,ctx:Operator):
    async with request.app.state.database.connection() as conn:
        ...
        if not row: raise DomainError("NOT_FOUND","Email was not found",status=404)
        ...
        if result.rowcount != 1: raise DomainError("RESTORE_UNAVAILABLE","The email is not in Trash or its 30-day restore period expired",status=409)
