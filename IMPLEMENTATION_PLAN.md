...
- [ ] **Phase 6: Gmail Integration (deprioritized)** — OAuth connect/callback exist. Tokens are encrypted at rest (`TOKEN_ENCRYPTION_KEY`), disconnect revokes the grant with Google and erases both tokens, and the address is no longer put in the redirect URL (2026-09-21). Remaining gap: `POST /gmail/connections/{id}/sync` only sets `state='syncing'` and fetches nothing. Not usable for real mailboxes until sync is built.
