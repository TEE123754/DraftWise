...
| **Database, Auth, Storage** | Supabase: 33 tables with Row Level Security, numbered migrations through `017` (`017` is applied on the live database; apply it before deploying, see [Deployment](#deployment)), a private `shipping-originals` bucket, email one-time-code sign-in. |
...
| GitHub Actions, backend job | Was failing on every push until 21 September because of a Linux-only pytest hook error; fixed, and green on the pushes since (`891cd54`, `57d1642`). Also green on this update (`db9b840`). |
...
| Hosted demo, fresh session, before this update | All 126 comparison emails read and compared in about **8 min 50 s** (3 hosted worker slots plus 3 laptop slots); end state 46 mismatches, 63 checked, 8 needing review, 2 unreadable documents; the first tour email was *Checked* within seconds; *Review with AI* completed in under a minute. |
| Hosted demo, fresh session, on the deployed update | All 126 comparison emails read and compared in about **3 min 7 s** (241 jobs, about 1.3 jobs per second) with 4 new hosted slots plus 3 laptop slots on old code; `email_001` *Checked* after 11 s; **identical end state**; Analytics showed the organizer score and the held-out AI score with no click; *Simulate Gmail fetch* reported "Fetched 520 emails … 0 new, 520 already in your workspace". `/ready` reported `hosted: 4, unknown: 6`, so the laptop workers showed up as such. |
...
- **Hosted demo speed depends on the database round trip.** Reading all 126 comparison emails took about 8 minutes 50 seconds before this update and about 3 minutes 7 seconds after it (see the demo section for the conditions, including laptop workers that helped). It still grows with the distance between the deployed service and the database, so set the Railway region next to Supabase.
